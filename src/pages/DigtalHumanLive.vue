...
const loadLiveRooms = async () => {
  try {
    ...
    liveStore.update(a)
    console.log('liveStore.livesList', liveStore.livesList)
    // 加载每个直播间的预览图
...
const queryIsDownloadable = async (list) => {
  ...
  if (list.length) {
    ...
    const promises = list.map(i => {
      ...
      return MusetalkUtils.getTaskStatus(i.id).then(r => {
...
const sendDownloadRequest = async id => {
   ...
   */
  MusetalkUtils.getRenderedViews(id)
    .then(async path => {
      console.log('已经拿到视频文件夹:', path)
      getDefaultPlaySequence(id)

      liveStore.update(liveStore.livesList.map(i => {
        if (i.id == id) {
    ...
    <div class="grid grid-cols-4 gap-6">
      <template v-if="filteredRooms.length">
        <div v-for="room in filteredRooms" :key="room.id"
          class="relative aspect-[3/4] rounded-lg overflow-hidden cursor-pointer group" @click="handleRoomClick(room)">
          <!-- 删除按钮 -->
          <div class="absolute top-3 right-3 z-10">
            <a-button type="primary" status="danger" shape="circle" size="mini"
              class="opacity-0 group-hover:opacity-100 transition-opacity" @click="(e) => handleDelete(room, e)">
              <template #icon>
                <icon-delete />
              </template>
            </a-button>
          ...
          <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent">
            <div class="absolute bottom-0 left-0 right-0 p-6">
              ...
              <div class="flex justify-between items-center">
                <a-tag :color="getStateColor(liveStore.livesList.find(i => i.id === room.id)?.state)" size="small">
