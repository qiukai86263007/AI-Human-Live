<script setup lang="ts">
  ...
  const filteredRooms = computed(() => {
    return liveRooms.value.filter(room => {
      ...
      // 状态类型过滤
      const matchType = selectedType.value === '全部' ? true : room.state === selectedType.value;
  ...
  const queryIsDownloadable = async list => {
    ...
    if (list.length) {
      ...
      const promises = list.map(i => {
        ...
        return MusetalkUtils.getTaskStatus(i.id)
          .then(r => {
            if (r.data) return Promise.resolve(r.data);
          ...
          .then(d => {
            if (d.completed_count == d.total_count) {
              ...
              //   }
              // }
              console.log('有待渲染项,sendDownloadRequest');
   ...
   */
    MusetalkUtils.getRenderedViews(id).then(async path => {
      console.log('已经拿到视频文件夹:', path);
      getDefaultPlaySequence(id);

      liveStore.update(
        liveStore.livesList.map(i => {
          if (i.id == id) {
            i.state = LiveRoomState.CREATED;
          }
          return i;
        })
      );
      await LiveBroadcastService.update(id, {
    ...
    // 下面确定视频顺序
    let defaultSequence = await getDefaultPlaySequence(id);
  };
  const getDefaultPlaySequence = async (id: string) => {
    // 暂定根据音频脚本的创建事件来排序
    const products = await LiveProductService.listByLiveId(id);
    let timeList: any[] = [];
    for (const product of products) {
      ...
      for (const script of scripts) {
        timeList.push({
          id: script.id,
          time: script.create_date,
        });
      }
    }
    let sequenced = timeList.sort((a, b) => {
      return new Date(a.time).getTime() - new Date(b.time).getTime();
    });
    console.log('timeList: ', timeList);
    console.log('sequenced: ', sequenced);
    return sequenced;
  };
  ...
  const handleRoomClick = room => {
    // 此处room为store.livesList中的对应项
    console.log('handleRoomClick roomId:', room.id);
    if (!room.id) return;
    if (room.state == LiveRoomState.EDITING) {
      router.push({
        path: '/live-room/edit', // 开始直播的页面 目前也在编辑页面里面
        query: { id: room.id, autoSelect: 'true' },
      });
    } else if (room.state == LiveRoomState.CREATING) {
      Message.info('正在渲染中，请稍等');
      // 目前的策略是  依然在编辑页中展示视频
    } else if (
      room.state == LiveRoomState.CREATED ||
      room.state == LiveRoomState.LIVE ||
      room.state == LiveRoomState.STOPPED
    ) {
      router.push({
        // path: '/live-room/play',    // 直播的页面
        path: '/live-room/edit',
        query: { id: room.id, autoSelect: 'true' },
      });
      // Message.info('你好')
...
<template>
  <div class="p-6">
    ...
    <div class="flex items-center gap-4 mb-6">
      <a-input v-model="searchText" placeholder="搜索直播间名称" class="w-64" allow-clear>
        <template #prefix>
          <icon-search />
        </template>
      </a-input>

      <a-select v-model="selectedType" :style="{ width: '120px' }" :options="typeOptions" />

      <a-range-picker v-model="dateRange" class="w-72" />
    ...
    <div class="grid grid-cols-4 gap-6">
      <template v-if="filteredRooms.length">
        <div
          ...
          @click="handleRoomClick(liveStore.livesList.find(i => i.id === room.id))"
        >
          <!-- 删除按钮 -->
          ...
          <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent">
            <div class="absolute bottom-0 left-0 right-0 p-6">
              ...
              <div class="text-gray-100 text-sm line-clamp-2 mb-4">
                {{ room.live_introduction }}
              </div>
              <div class="flex justify-between items-center">
                <a-tag
                  :color="getStateColor(liveStore.livesList.find(i => i.id === room.id)?.state)"
                  size="small"
                >
                  {{ getStateText(liveStore.livesList.find(i => i.id === room.id)?.state) }}
                </a-tag>
