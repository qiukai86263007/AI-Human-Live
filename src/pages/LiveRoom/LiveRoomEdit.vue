<script setup lang="ts">
  ...
  import SpeechAttributeService, {
    SpeechAttributeRecord,
  } from '../../service/SpeechAttributeService';
  ...
  const shouldDisableFeatures = computed(() => {
    ...
    return state === LiveRoomState.CREATED || state === LiveRoomState.LIVE;
  });
  ...
  const deleteCategory = async (categoryId: string) => {
    ...
    try {
      ...
      // 从界面数据中移除
      currentProduct.value.questionCategories = currentProduct.value.questionCategories.filter(
        c => c.id !== categoryId
      );
  ...
  const handleStartLive = async () => {
    ...
    try {
      const state = liveRoom.value?.state as LiveRoomState;
      const newState: LiveRoomState =
        state === LiveRoomState.CREATED ? LiveRoomState.LIVE : LiveRoomState.CREATED;
      const actionText = newState === LiveRoomState.LIVE ? '启动' : '停止';

      // 如果是要开始直播，显示确认对话框
      if (newState === LiveRoomState.LIVE) {
        Modal.confirm({
          title: '开启场控',
          content: '是否同时开启场控？',
          okText: '是',
  ...
  const updateLiveState = async (newState: LiveRoomState) => {
    const liveId = route.query.id as string;
    try {
      Message.loading(`正在${newState === LiveRoomState.LIVE ? '启动' : '停止'}直播...`);
      ...
      await loadLiveRoom();
      Message.success(`直播已${newState === LiveRoomState.LIVE ? '启动' : '停止'}`);
    } catch (error) {
  ...
  const handleSceneControlConfirm = async (settings: any) => {
    try {
      ...
      // 更新直播状态为运行中
      await updateLiveState(LiveRoomState.LIVE);
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (
        currentProduct.value &&
        selectedProducts.value.includes(currentProduct.value.product_id!)
      ) {
  ...
  const handleStartClone = async () => {
    ...
    try {
      ...
      checkNecessary();
      // TODO 如果根本没有产品 不允许点击克隆按钮
      products = await LiveProductService.listByLiveId(liveId);
      getAudio = async () => {
        ...
        await window.$mapi.log.info('语音克隆完成，createRenderTask...');
      };
      await getAudio();
      console.log('完成getAudio');
      createRenderTask = async () => {
        ...
        for (let product of products) {
          ...
          for (const script of scripts) {
            ...
            await window.$mapi.file.copy(fullAudioPath, fullPath + `/${script.id}.wav`, {
              isFullPath: true,
            });
          }
        }
        ...
        console.log('urlOfZiped: ', urlOfZiped);
        let params = {
          // 接口所需参数
          clientId: '',
          userId: '',
  ...
  const filteredQuestionCategories = computed(() => {
    ...
    return currentProduct.value.questionCategories
      .map(category => ({
        ...category,
  ...
  // 在 script 部分添加
  const publicScreenComments = ref<
    Array<{ username: string; content: string; time: string; id: string }>
  >([]);

  // 添加处理新弹幕的方法
  const handleNewComments = (
    newComments: Array<{ username: string; content: string; time: string; id: string }>
  ) => {
        ...
        <div class="flex-1 overflow-y-auto">
          ...
          <template v-else>
            <div
              ...
              :key="product.id"
              class="px-4 py-3 cursor-pointer hover:bg-[#E5E6EB]"
              :class="{
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <div class="w-12 h-12 flex-shrink-0">
                  <img
                    v-if="product.product_backroud"
                    :src="product.product_backroud"
      ...
      <div class="flex-grow p-4">
        ...
        <div v-if="currentTab === '主播选择'" class="flex flex-col h-full">
          ...
          <div class="w-[600px] mx-auto">
            <div
              class="aspect-video rounded-lg flex items-center justify-center bg-gray-50 overflow-hidden"
            >
              <template v-if="selectedAnchor">
                <img
                  :src="selectedAnchor.anchor_backgroud"
          ...
          <div class="mt-4 flex justify-center">
            <a-button
              ...
              type="primary"
              :status="liveRoom?.state === LiveRoomState.CREATED ? 'success' : 'danger'"
              ...
              <div>
                ...
                <a-tabs>
                        ...
                        <div class="flex items-center gap-4">
                          ...
                          <div>
                            <div class="mb-1">
                              ...
                              {{
                                selectedVoice?.language_id === 1
                                  ? '普通话'
                                  : selectedVoice?.language_id === 2
                                    ? '英语'
                                    : selectedVoice?.language_id === 3
                                      ? '日语'
                                      : '未知'
                              }}
                            </div>
                          ...
                          <div class="bg-white rounded-lg p-4 shadow">
                            ...
                            <div class="mb-4 flex gap-2">
                              <a-input
                                ...
                                class="flex-grow"
                              />
                              <a-button type="primary" @click="generateAiContent"> 发送 </a-button>
                            </div>
                  ...
                  <a-tab-pane key="audio" title="音频台词编辑">
                    <div class="bg-white rounded-lg p-4 shadow">
                      <!-- 音频编辑相关内容 -->
                      <div class="text-gray-400">音频编辑功能开发中...</div>
                    </div>
                  </a-tab-pane>
                </a-tabs>
              </div>
          ...
          <div class="flex flex-grow -mt-4">
                  ...
                  >
                    ...
                    <div class="space-y-2 pl-4">
                      <div v-for="qa in category.qas" :key="qa.id" class="flex flex-col gap-2 py-2">
                        <div class="flex items-center justify-between">
                          ...
                          <div class="flex items-center gap-2">
                            ...
                            <a-button
                              ...
                              size="mini"
                              status="danger"
                              @click="deleteQA(category.id, qa.id)"
                            >
                              <template #icon>
                                <IconDelete />
                              </template>
                        ...
                        <div class="text-gray-500 pl-4">回答: {{ qa.answer }}</div>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="w-96 flex-shrink-0 border-l border-gray-800 p-4 min-h-full">
              ...
              <div class="bg-white rounded-lg p-4 shadow">
                ...
                <div class="mb-4">
                  <a-checkbox :model-value="true" :disabled="true"> 关键词问答 </a-checkbox>
                </div>
                <div class="mb-4">
                  回复设置
                  <div class="flex items-center gap-4">
                ...
                <div class="flex justify-end">
                  <a-button type="primary"> 保存当前设置 </a-button>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div v-else-if="currentTab === '商品讲解'">商品讲解内容</div>
      </div>

      <!-- 右侧主播列表 -->
      <div
        v-if="currentTab === '主播选择'"
        class="w-96 flex-shrink-0 border-l border-gray-800 pl-4 pt-4 min-h-full"
      >
            ...
            <template v-if="filteredAnchors.length">
              ...
              >
                <div class="aspect-[3/4] rounded-lg overflow-hidden relative">
                  <img
                    :src="anchor.anchor_backgroud"
                    :alt="anchor.anchor_name"
                    class="w-full h-full object-cover"
                  />
                  <div
                    v-if="selectedAnchor?.id === anchor.id"
        ...
        <template v-else>
          <div class="h-full">
            <div class="text-lg font-medium mb-4">AI智能互动</div>
            <!-- AI智能互动设置表单 -->
            <div>
              <!-- 直播平台选择 -->
              <div class="mb-4">
            ...
            <div>
              <div class="mb-6">
                ...
                <div class="h-96 bg-[#2A2B3C] rounded p-4 overflow-y-auto">
                  <template v-if="publicScreenComments.length > 0">
                    <div
                      v-for="comment in publicScreenComments"
                      :key="comment.id"
              ...
              <div class="flex gap-4 justify-center">
                ...
                />
                <a-button type="primary" @click="sendComment">
                  <template #icon>
                    <IconSend />
                  </template>
                  发送
