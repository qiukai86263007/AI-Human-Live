...
const handleStartLive = async () => {
  ...
  try {
    const state = liveRoom.value?.state as LiveRoomState;
    const newState: LiveRoomState = state === LiveRoomState.CREATED ? LiveRoomState.LIVE : LiveRoomState.CREATED;
...
<template>
  <div class="flex flex-col min-h-screen">
    ...
    <div class="flex-1 flex">
      ...
      <div class="w-60 flex-shrink-0 border-r border-gray-800 flex flex-col">
        ...
        <div class="p-4">
          <a-button type="primary" class="w-full mb-2" status="primary" :disabled="shouldDisableFeatures"
          ...
          <a-button type="primary" class="w-full" :disabled="shouldDisableFeatures" @click="handleStartClone">
            <template #icon>
              <IconPlayCircle />
            </template>
            开始克隆
      ...
      <div class="flex-grow p-4">
        ...
        <div v-if="currentTab === '主播选择'" class="flex flex-col h-full">
          ...
          <div class="mt-4 flex justify-center">
            <a-button type="outline" :disabled="!currentProduct || !selectedAnchor || shouldDisableFeatures"
          ...
          <div class="h-full">
            ...
            <div>
              ...
              <div class="mb-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                  <a-checkbox :model-value="rulesDemo.productQA" @update:model-value="(val: boolean | (string | number | boolean)[]) => {
                    rulesDemo.productQA = Boolean(val);
                    console.log('产品问答复选框状态更新:', val);
                  }" :disabled="!canEditAISettings">产品问答</a-checkbox>
                  <a-checkbox :model-value="rulesDemo.giftThanks" @update:model-value="(val: boolean | (string | number | boolean)[]) => {
                    rulesDemo.giftThanks = Boolean(val);
                  }" :disabled="!canEditAISettings">礼物感谢</a-checkbox>
                  <a-checkbox :model-value="rulesDemo.welcomeGuide" @update:model-value="(val: boolean | (string | number | boolean)[]) => {
                    rulesDemo.welcomeGuide = Boolean(val);
                  }" :disabled="!canEditAISettings">定时引导</a-checkbox>
                  <a-checkbox :model-value="rulesDemo.popularActivity" @update:model-value="(val: boolean | (string | number | boolean)[]) => {
                    rulesDemo.popularActivity = Boolean(val);
                  }" :disabled="!canEditAISettings">人气互动</a-checkbox>
                </div>
                <div class="mt-2">
            ...
            <div>
              ...
              <div class="flex gap-4 justify-center">
                <a-input class="flex-1" v-model="commentText" placeholder="请输入消息内容" :maxLength="40"
  ...
  <!-- 场控设置弹窗 -->
  <SceneControlDialog ref="sceneControlDialogRef" v-model="showSceneControlDialog" :roomId="roomIdDemo"
    :welcome-guide="rulesDemo.welcomeGuide" :productQA="rulesDemo.productQA" @confirm="handleSceneControlConfirm"
    @onNewComments="handleNewComments" @update:visible="(val) => !val && handleSceneControlClose()" />
