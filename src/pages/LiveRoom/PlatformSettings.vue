    ...
    <div class="flex h-[600px]">
      ...
      <div class="w-40 border-r border-gray-200">
        <div
          ...
          :class="
            currentTab === tab.key ? 'bg-blue-50 text-blue-500' : 'text-gray-500 hover:bg-gray-50'
          "
      ...
      <div class="flex-1 p-4">
        ...
        <div v-if="currentTab === 'global'" class="h-full">
          <div class="flex flex-col h-full">
            <div class="space-y-6 flex-grow">
              ...
              <div>
                ...
                <div class="flex items-center gap-2">
                  <a-input-number v-model="settings.replyDelay" :min="0" :max="100" class="w-24" />
                  <span>每隔*秒发送一次</span>
                </div>
              </div>
            ...
            <div class="flex justify-end mt-6 pt-4 border-t">
              <a-button type="primary" @click="handleSave"> 保存当前设置 </a-button>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="currentTab === 'timing'" class="h-full">
          <div class="flex flex-col h-full">
                ...
                <div>
                  <div class="rounded-lg p-4 h-[400px] overflow-y-auto">
                    <template v-if="getGuideList(currentGuideType).length">
                      <div
                        v-for="guide in getGuideList(currentGuideType)"
                        :key="guide.id"
                    ...
                    <template v-else>
                      <div class="text-gray-400 text-center py-20">暂无引导内容</div>
                    </template>
                  </div>
                </div>
              </div>
            ...
            <div class="flex justify-end items-center h-16 mt-4 pt-4 border-t border-gray-200">
              <a-button type="primary" @click="saveTimingSettings"> 保存当前设置 </a-button>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="currentTab === 'popularity'" class="h-full">
          <div class="flex flex-col h-full">
                ...
                <div>
                  ...
                  <div class="rounded-lg p-4 h-[400px] overflow-y-auto">
                    <template v-if="popularitySettings.like.rules.length">
                      <div
                        v-for="rule in popularitySettings.like.rules"
                        :key="rule.id"
                        class="border border-gray-600 rounded-lg p-4 mb-3 last:mb-0"
                      >
                        <div class="flex items-center justify-between">
                          <div>
                            <div class="flex items-center gap-2">
                              <a-input-number v-model="rule.count" :min="0" class="w-24" />
                              <span>时，互动内容</span>
                            </div>
                          </div>
                          <div class="flex items-center gap-2">
                    ...
                    <template v-else>
                      <div class="text-gray-400 text-center py-20">暂无触发规则</div>
                    </template>
                  </div>
                </div>
              </div>
              ...
              <div class="flex-grow">
                ...
                <div class="rounded-lg p-4 h-[400px] overflow-y-auto">
                  <template v-if="popularitySettings.online.rules.length">
                    <div
                      v-for="rule in popularitySettings.online.rules"
                      :key="rule.id"
                      class="border border-gray-600 rounded-lg p-4 mb-3 last:mb-0"
                    >
                      <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2">
                          ...
                          <a-button
                            ...
                            size="mini"
                            status="danger"
                            @click="deleteOnlineRule(rule.id)"
                          >
                            <template #icon>
                              <icon-delete />
                            </template>
                  ...
                  <template v-else>
                    <div class="text-gray-400 text-center py-20">暂无互动内容</div>
                  </template>
                </div>
              </div>
            </div>
            ...
            <!-- 底部面板 -->
            <div class="flex justify-end items-center h-16 mt-4 pt-4 border-t border-gray-200">
              <a-button type="primary" @click="savePopularitySettings"> 保存当前设置 </a-button>
            </div>
          </div>
        </div>
        ...
        <div v-else-if="currentTab === 'gift'" class="h-full">
          <div class="flex flex-col h-full">
              ...
              <div class="w-[400px] flex-shrink-0">
                ...
                <div class="mb-4">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <a-input-number v-model="giftSettings.smallAmountMoney" :min="0" class="w-24" />
            ...
            <!-- 底部面板 -->
            <div class="flex justify-end items-center h-16 mt-4 pt-4 border-t border-gray-200">
              <a-button type="primary" @click="saveGiftSettings"> 保存当前设置 </a-button>
            </div>
          </div>
        </div>
      </div>
    ...
    >
      <div class="space-y-4">
        <div class="flex items-center gap-2">
          ...
          <a-input-number v-model="editingRule.count" :min="0" class="w-24" />
...
<script setup lang="ts">
  ...
  import OverallSituationConfigService, {
    OverallSituationConfigRecord,
  } from '../../service/OverallSituationConfigService';
    ...
    try {
      ...
      if (config) {
        settings.value = {
          ...
          priorities: {
            regularInteraction:
              config.regular_interaction_priority ?? defaultSettings.priorities.regularInteraction,
          ...
          naturalLanguageSwitch:
            config.globalnatural_language_switch ?? defaultSettings.naturalLanguageSwitch,
  ...
  const deleteGuide = (guideId: number) => {
    ...
    if (tab === 'gift') {
      ...
      if (index > -1) {
        ...
        if (key) {
          timingSettings.value.contents[key] = JSON.stringify(
            guideList.value[currentGuideType.value]
          );
        }
      }
    }
  ...
  const loadGiftSettings = async () => {
    ...
    try {
      ...
      if (config) {
        ...
        giftSettings.smallAmountMoney =
          config.small_amount_money ?? defaultGiftSettings.smallAmountMoney;
        giftSettings.strengthenThankEnable = config.strengthen_thank_enable ?? 1;
        giftSettings.rules = {
          small: config.small_amount_thank_contents
            ? JSON.parse(config.small_amount_thank_contents)
            : [],
          large: config.big_amount_thank_contents
            ? JSON.parse(config.big_amount_thank_contents)
            : [],
          strengthen: config.strengthen_thankcontents
            ? JSON.parse(config.strengthen_thankcontents)
            : [],
        };
      }
    } catch (error) {
  ...
  // 当前编辑的规则
  const editingRule = ref<{ id: number; count: number; content: string }>({
    id: 0,
    count: 0,
    content: '',
  });
