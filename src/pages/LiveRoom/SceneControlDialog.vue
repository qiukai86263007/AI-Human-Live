...
<script setup lang="ts">
  ...
  const startCollection = () => {
    ...
    `;
      ...
      webviewRef.value
        ?.executeJavaScript(platform.value === 'kuaishou' ? kuaishouScript : douyinScript)
        .then((result: any) => {
          if (Array.isArray(result) && result.length > 0) {
            const currentTime = new Date().getTime();
            const newComments = result.filter(comment => {
              const isDuplicate = comments.value.some(
                existing =>
  ...
  const startAutoReply = async () => {
    ...
    if (platform.value === 'kuaishou') {
      ...
      const handleProductQA = async () => {
        const unrepliedComments = comments.value.filter(
          comment => !repliedComments.has(comment.id)
        );
      ...
      try {
        ...
        if (props.welcomeGuide) {
          ...
          // 设置定时引导的定时器
          autoReplyInterval.value = window.setInterval(handleWelcomeGuide, intervalTime);
        ...
        // 发生错误时分别设置默认间隔
        if (props.welcomeGuide) {
          autoReplyInterval.value = window.setInterval(handleWelcomeGuide, 10000);
        }
        if (props.productQA) {
          productQAInterval.value = window.setInterval(handleProductQA, 1000);
        }
  ...
  LIMIT 1`;
    ...
    if (selectedGroup?.contents.length) {
      ...
      const selectedContent =
        selectedGroup.contents[Math.floor(Math.random() * selectedGroup.contents.length)];
      return {
  ...
  const getProductQAReply = async (commentContent: string) => {
    ...
    try {
      ...
      for (const qa of qaResults) {
        ...
        const replies = Array.isArray(qa.replys) ? qa.replys : JSON.parse(qa.replys || '[]');

        console.log('正在匹配问题组:', likeProblems);

        // 查找匹配的问题
        const matchIndex = likeProblems.findIndex(problem => commentContent.includes(problem));
  ...
  watch(
    () => props.productQA,
    (newValue, oldValue) => {
      console.log('SceneControlDialog - productQA 属性变化:', oldValue, '->', newValue);
      if (platform.value === 'kuaishou') {
        if (newValue) {
          console.log('产品问答功能开启，启动自动回复');
          startAutoReply();
        } else {
          console.log('产品问答功能关闭');
          if (productQAInterval.value) {
            clearInterval(productQAInterval.value);
            productQAInterval.value = null;
          }
  ...
  watch(
    () => props.welcomeGuide,
    (newValue, oldValue) => {
      console.log('welcomeGuide 属性变化:', oldValue, '->', newValue);
      if (platform.value === 'kuaishou') {
        if (newValue) {
          console.log('定时引导功能开启，启动自动回复');
          startAutoReply();
        } else {
          console.log('定时引导功能关闭');
          if (autoReplyInterval.value) {
            clearInterval(autoReplyInterval.value);
            autoReplyInterval.value = null;
          }
