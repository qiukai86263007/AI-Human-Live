<template>
  <div class="p-6">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center">
            ...
            <div class="relative w-16 h-16 mr-4">
              ...
              <div
                v-else
                class="w-full h-full rounded-lg bg-[#1D1E2B] flex items-center justify-center"
              >
    ...
    <a-modal
      v-model:visible="showCloneVoiceDialog"
      title="新建克隆声音"
      width="500"
        ...
        <a-tabs default-active-key="1">
          <a-tab-pane key="1" title="火山引擎">
            <div class="mb-4">
              ...
              <div>
                <span
                  >上传一段音频文件，用于克隆声音。建议上传音频时长为10-30秒，音频质量大于音频的时长，避免多人对话、明显杂音、噪音等情况</span
                >
              </div>
    ...
    <a-modal
      v-model:visible="showKeyConfigDialog"
      title="配置密钥"
      width="500"
      @ok="handleSaveKeyConfig"
          ...
          <a-tab-pane key="1" title="火山密钥配置">
            <div class="mb-6">
              ...
              <div class="mb-4">
                ...
                <div>
                  <a-input
                    v-model="keyConfig.access_key_secret"
    ...
    <a-modal
      v-model:visible="showSelectVoiceDialog"
      title="选择主播声音"
      width="500"
      @ok="handleSelectVoice"
    ...
    >
      <div class="select-voice-form">
        <a-select v-model="selectedVoiceId" style="width: 100%" @change="handleSelectVoice">
          <a-option v-for="voice in availableVoices" :key="voice.id" :value="voice.id">{{
            voice.name
          }}</a-option>
        </a-select>
      </div>
...
<script setup lang="ts">
      ...
      if (statusData.BaseResp.StatusCode === 0) {
        if (statusData.status === 2 && statusData.demo_audio) {
          try {
            ...
            // 确保目录存在
            const fullSubDir = await window.$mapi.file.fullPath(
              join('audio', 'sound_anchor_cloned')
            );
  ...
  const loadSoundList = async () => {
    try {
      const list = await AudioCharacterService.list();
      const soundListWithStatus = await Promise.all(
        list.map(async item => {
          try {
  ...
  const handleSaveKeyConfig = async () => {
    try {
      // 验证所有字段都已填写
      if (
        !keyConfig.value.app_key ||
        !keyConfig.value.access_key_secret ||
        !keyConfig.value.hsKeyid ||
        !keyConfig.value.hsAccessKey
      ) {
      ...
      try {
        if (!isEditing.value) {
          ...
          await train(
            config.app_key,
            config.access_key_secret,
            PathManager.fromStoragePath(audioPath),
            voiceId.value
          );

          // 获取克隆音频
          clonedAudioPath = await getClonedAudio(
            config.app_key,
            config.access_key_secret,
            voiceId.value,
            name.value
