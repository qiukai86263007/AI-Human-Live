      ...
      <div class="">
        <div v-if="!modelInfo.name">
          <div class="">
            <div v-if="0" class="w-1/2 px-3">
              <div>
                <img
                  class="w-32 h-32 object-contain m-auto"
                  src="./../../assets/image/server-file.svg"
                />
              </div>
              <div>
                <a-button @click="doSelectFile" class="block w-full" :loading="loading">
                  <template #icon>
                    <icon-file />
                  </template>
                  {{ t('选择模型ZIP文件') }}
            ...
            <div class="px-3">
              ...
              <div>
                <a-button @click="doSelectFileDir" class="block w-full" :loading="loading">
                  <template #icon>
                    <icon-folder />
                  </template>
                  {{ t('选择模型服务目录') }}
        ...
        <div v-else>
          ...
          <div class="pt-4 flex items-center">
            <div>
              <a-button class="mr-2" type="primary" :loading="isImporting" @click="doSubmit">
                <template #icon>
                  <icon-check />
                </template>
                {{ $t('确认提交') }}
